...
**Specification version:** 0.4.14  
**Codebase version:** 0.4.14  
...
## v0.4.8 Component Contour Overlay

Geometry Viewer includes a **Show components** checkbox. When enabled, the viewer draws component contours only, without filled component bodies, and places the reference designator text at the center of each component contour. Component reference text is collected from common fabrication/silkscreen layers and contours are estimated from common courtyard/fabrication layers. The overlay is a visual inspection aid and is not used by clearance calculations.

## v0.4.9 JSON Settings Profiles

The GUI includes a **Settings** tab for loading and exporting project/manual settings as JSON. The settings profile schema is `odb_clearance_analyzer_settings` and is written by default as `analysis_settings.json`.

The profile contains all manual settings needed to reproduce the same configuration:

- ODB++ input path and output folder
- critical threshold
- `$NONE$` inclusion
- Effective Net-to-Net matrix setting
- debug limit
- CTI, base pollution degree, altitude
- IPC layer roles
- IEC per-layer pollution-degree assignments
- external conformal coating state
- metallic particle size
- voltage export flags

Every analysis automatically writes `analysis_settings.json` to the output folder and includes it in `net_to_net_report_package.zip`.

## v0.4.9 Component Overlay Fix

Geometry Viewer **Show components** now calls a dedicated toggle handler. When enabled, the viewer refits to include component contours and can draw component contours/reference designators even when no net is selected. The component overlay remains visual-only and is not used for clearance calculations.

## v0.4.14 IEC 61558 / IEC 62368 Isolation Tab

The GUI includes an **IEC 61558 / IEC 62368** tab for isolation-barrier screening. The tab provides an isolated-side net group input where the user enters one or more net names belonging to the isolated side of a transformer or isolation barrier. Each isolated-side net is checked against every other net present on each same-layer copper geometry.

Adjustable parameters include standard/profile label, insulation type, working voltage, peak/recurring voltage, impulse voltage, overvoltage category, pollution degree, material group/CTI class, CTI, altitude, required clearance, required creepage and safety factor. Required clearance and creepage are manual fields so the user can enter values from the applicable exact standard edition/table. The governing required spacing is `max(required_clearance_mm, required_creepage_mm) * safety_factor`.

The result table displays group, layer, isolated net, other net, measured same-layer spacing, required clearance, required creepage, governing required spacing, margin and pass/fail. Rows can be opened in Geometry Viewer. Isolation tab variables are stored in the JSON settings profile under `isolation_settings` and are included in automatic `analysis_settings.json` exports.

Limitations: this is a same-layer ODB++ copper-geometry screening helper. It does not certify compliance, does not apply hidden standard table values automatically, and does not evaluate 3D component-body distances, bobbin/transformer construction, slots routed through the PCB unless represented in copper geometry, or vertical stackup dielectric distances.
