...
**Specification version:** 0.4.11  
**Codebase version:** 0.4.11  
...
## v0.4.11 UI and Blocker-Dot Visualization

The GUI combines the top input/output and analysis controls into one compact **Input, output and analysis** card. The critical threshold label is emphasized in bold, and the threshold input uses a light-yellow background. Diagnostic settings are centralized in the **Settings** tab: **Debug rows** is moved there, and **Clearance gradient** is moved there and enabled by default.

Geometry Viewer adds blocker-dot visualization. When a straight minimum-spacing line between selected copper elements intersects other copper, the viewer draws magenta dots at those intersection points. These dots show which other copper pieces lie on the straight-line path and are excluded/blocked when interpreting Effective Net-to-Net distance.
