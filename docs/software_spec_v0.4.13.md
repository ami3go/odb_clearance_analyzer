...
**Specification version:** 0.4.13  
**Codebase version:** 0.4.13  
...
## v0.4.13 Geometry Viewer Minimum-Clearance Zoom

Geometry Viewer includes a **Zoom min 2/3** button. The button requires selected Net A and Net B on the active layer. It computes the exact Shapely nearest points between the two selected net geometries, centers the view on the midpoint of that minimum-clearance line, and sets the scale so the line length uses approximately 2/3 of the available canvas length. Available canvas length is defined as the smaller of current canvas width and height. For zero-clearance overlap/touching conditions, the viewer centers on the representative overlap point and opens a small local window.
